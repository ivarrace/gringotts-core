...
public class GroupService {
    ...
    public List<Group> findByAccountancyKeyAndType(String accountancyKey, GroupType groupType) {
        return groupRepositoryPort.findAllByTypeInAccountancy(groupType, accountancyKey);
    }

    public Group create(Group group) {
        Optional<Group> existing =
                groupRepositoryPort.findByKeyAndTypeInAccountancy(group.getKey(),
                        group.getType(), group.getAccountancy().getKey());
        if(existing.isPresent()){
            throw new ObjectAlreadyRegisteredException(group.getKey());
        }
        Accountancy accountancy =
                accountancyService.findByKey(group.getAccountancy().getKey());
    ...
    public Group findByKey(String groupKey, String accountancyKey, GroupType groupType) {
        return groupRepositoryPort.findByKeyAndTypeInAccountancy(groupKey, groupType, accountancyKey)
                .orElseThrow(() -> new ObjectNotFoundException(groupKey));
    }

    public Group modify(String groupKey, Group group) throws ObjectNotFoundException {
        Group existing = this.findByKey(groupKey,
    ...
    public void delete(String accountancyKey, String groupKey,
                       GroupType groupType) throws ObjectNotFoundException {
        Group existing = this.findByKey(groupKey,
                accountancyKey, groupType);
