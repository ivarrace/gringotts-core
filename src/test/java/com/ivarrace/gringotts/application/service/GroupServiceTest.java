...
class GroupServiceTest {
    ...
    public void init() {
        ...
        groupService =
                new GroupService(groupRepositoryMock, accountancyServiceMock);
    }
    ...
    void findAll() {
        ...
        verify(groupRepositoryMock, times(1)).findAllByTypeInAccountancy(group.getType(), group.getAccountancy().getKey());
    ...
    void findByKey() {
        ...
        when(groupRepositoryMock.findByKeyAndTypeInAccountancy(group.getKey(),
                group.getType(), group.getAccountancy().getKey())).thenReturn(Optional.of(group));
        Group result = groupService.findByKey(group.getKey(),
                group.getAccountancy().getKey(), group.getType());
        assertNotNull(result);
        assertAll("Expected values",
                () -> assertEquals(group.getId(), result.getId()),
                () -> assertEquals(group.getKey(), result.getKey()),
                () -> assertEquals(group.getCreatedDate(),
                        result.getCreatedDate()),
                () -> assertEquals(group.getLastModified(),
                        result.getLastModified()),
                () -> assertEquals(group.getName(), result.getName()),
                () -> assertEquals(
                        group.getCategories().size(),
                        result.getCategories().size()),
                () -> assertEquals(group.getAccountancy().getKey(),
                        result.getAccountancy().getKey())
        );
        verify(groupRepositoryMock, times(1)).findByKeyAndTypeInAccountancy(group.getKey(),
                group.getType(), group.getAccountancy().getKey());
        verifyNoMoreInteractions(groupRepositoryMock, accountancyServiceMock);
    }
    ...
    void create() {
        ...
        when(groupRepositoryMock.findByKeyAndTypeInAccountancy(group.getKey(),
                group.getType(), existingAccountancy.getKey())).thenReturn(Optional.empty());
        ...
        verify(groupRepositoryMock, times(1)).findByKeyAndTypeInAccountancy(group.getKey(),
                group.getType(), existingAccountancy.getKey());
    ...
    void create_alreadyExists() {
        ...
        group.setAccountancy(existingAccountancy);
        when(groupRepositoryMock.findByKeyAndTypeInAccountancy(group.getKey(),
                group.getType(), existingAccountancy.getKey())).thenReturn(Optional.of(group));
        when(accountancyServiceMock.findByKey(existingAccountancy.getKey())).thenReturn(existingAccountancy);
        when(groupRepositoryMock.save(group)).thenReturn(group);
    ...
    void modifyByKey() {
        ...
        when(groupRepositoryMock.findByKeyAndTypeInAccountancy(existing.getKey(),
                modified.getType(), modified.getAccountancy().getKey())).thenReturn(Optional.of(existing));
        ...
        verify(groupRepositoryMock, times(1)).findByKeyAndTypeInAccountancy(existing.getKey(),
                modified.getType(), modified.getAccountancy().getKey());
